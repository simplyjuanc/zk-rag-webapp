...
def pipeline_db_storage_callback_factory() -> Callable[[PipelineResult], Awaitable[None]]:
    async def pipeline_db_storage_callback(result: PipelineResult) -> None:
        ...
        file_meta = getattr(result.document.metadata, "file_metadata", None) if result.document.metadata else None
        front_meta = getattr(result.document.metadata, "frontmatter_metadata", None) if result.document.metadata else None
        ...
        doc_db_model = DocumentDB(
            ...
            processed_at=str(result.document.processed_at) if result.document.processed_at else None,
        )
            ...
            if result.chunks:
                for chunk in result.chunks:
                    chunk_data = DocumentChunkDB(
                        ...
                        embedding_created_at=str(getattr(chunk, 'embedding_created_at', None)),
                    )
