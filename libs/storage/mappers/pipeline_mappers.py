...
def map_processed_document_to_db(processed_doc: ProcessedDocument) -> DocumentDB:
    ...
    if frontmatter and frontmatter.author:
        author_str = json.dumps(frontmatter.author) if isinstance(frontmatter.author, list) else str(frontmatter.author)

    category_str = None
    if frontmatter and frontmatter.category:
        category_str = json.dumps(frontmatter.category) if isinstance(frontmatter.category, list) else str(frontmatter.category)

    document_type_str = None
    if frontmatter and frontmatter.type:
        document_type_str = json.dumps(frontmatter.type) if isinstance(frontmatter.type, list) else str(frontmatter.type)
    ...
    return DocumentDB(
        ...
        content_created_at=file_metadata.content_created_at.isoformat() if file_metadata else None,
        content_modified_at=file_metadata.content_modified_at.isoformat() if file_metadata else None,
...
def map_pipeline_chunks_to_db(chunks: List[EmbeddedChunk], document_id: str) -> List[DocumentChunkDB]:
